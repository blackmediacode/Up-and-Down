...
from src.Kali import Kaldi
from src.Bunna import Bunna
...
class Game:
    ...
    def loop(self):
        ...
        snake = Kaldi(self.display)
        apple = Bunna(self.display)
        ...
        while True:
            ...
            # Move and Re-Draw Kaldi
            snake.move(x_change, y_change)
            snake_rect = kaldi.draw()
            snake.draw_body()
            ...
            # Detect collision with bunna
            if apple_rect.colliderect(Kaldi_rect):
                apple.randomize()
                self.score += 1
                kalid.eat()
            ...
            # Collide with Self
            if len(kaldi.body) >= 1:
                for cell in kaldi.body:
                    if kaldi.x_pos == cell[0] and kaldi.y_pos == cell[1]:
                        self.loop()
            ...
            title = font.render('Person', True, Config['colors']['white'])
